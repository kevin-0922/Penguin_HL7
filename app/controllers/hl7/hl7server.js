...
class Hl7Server {
  constructor() {
    ...
    // MLLP封裝常量
    this.MLLP_HEADER = Buffer.from([0x0b]);  // <VT>
    this.MLLP_TRAILER = Buffer.from([0x1c, 0x0d]);  // <FS><CR>
  }
  ...
  async handleRequest(req, res) {
    try {
      ...
      const responseData = await this.sendViaSocket(this.config.remoteHost, this.config.remotePort, hl7Message);
  ...
  // 使用Socket直接發送MLLP消息
  async sendViaSocket(host, port, message) {
    return new Promise((resolve, reject) => {
      ...
      client.on('data', (data) => {
        ...
        const trailerIndex = responseData.indexOf(this.MLLP_TRAILER);
        if (trailerIndex >= 0) {
          ...
          // 檢查回應是否符合MLLP格式並解析
          if (responseData[0] === this.MLLP_HEADER[0]) {
            // 提取HL7消息內容 (去除MLLP包裝)
      ...
      client.on('timeout', () => {
        client.destroy();
        reject(new Error(`連接超時，超過 ${this.config.timeout}ms 未收到回應`));
      });
      ...
      client.on('close', () => {
        if (responseReceived) {
          // 處理接收到的數據
          const trailerIndex = responseData.indexOf(this.MLLP_TRAILER);
          if (responseData[0] === this.MLLP_HEADER[0] && trailerIndex > 0) {
            // 提取HL7消息內容 (去除MLLP包裝)
            const hl7Content = responseData.slice(1, trailerIndex).toString('utf-8');
            resolve(hl7Content);
          } else {
