...
const { convertO19ToDicom } = require('../dicom/hl7ToDicomConverter');
...
async function handleOrmO19(message) {
  ...
  try {
    ...
    try {
      ...
      // 轉換為 DICOM 並存儲到 raccon 資料庫
      console.log('🔄 開始轉換為 DICOM 格式...');
      const dicomResult = await convertO19ToDicom(hl7JsonForDicom, orderId);
      if (dicomResult) {
        console.log('✅ DICOM 轉換成功並已存儲到 raccon 資料庫');
      } else {
...
async function insertOrmO19Order(order) {
  try {
    ...
    // 注意：DICOM 轉換已經在 handleOrmO19 函數中完成，這裡不需要重複調用
    console.log('✅ 訂單已成功插入到 omg_o19_orders 表');
