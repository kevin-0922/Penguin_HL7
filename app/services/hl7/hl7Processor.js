...
async function processHL7Message(message) {
  ...
  try {
    ...
    console.log('清理後的 HL7 訊息:', cleanMsg);

    console.log('handleOmiO23:', handleOmiO23);
    if (cleanMsg.includes('OMI^O23')) {
      ack = await handleOmiO23(cleanMsg);
    } else if (cleanMsg.includes('ORM^O19')) {
      ack = await handleOrmO19(cleanMsg);
    } else {
