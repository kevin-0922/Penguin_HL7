  ...
  return (
    <details className={`bg-white p-6 rounded-lg shadow-md mb-6 ${className}`}>
      ...
      <div className="mt-4 text-sm text-gray-500">
        <p>註：<span className="text-red-500">*</span> {t('common.requiredNote')}</p>
      </div>
    </details>
  );
};
...
export const FormField = ({ label, enName = '', fieldNotation = '', required, children, fullWidth = false }) => {
  ...
  let displayLabel;
  if (isEn) {
    displayLabel = enName + (showFieldNotation && fieldNotation ? ` (${fieldNotation})` : '');
  } else {
    displayLabel = label
      + (enName ? ` (${enName})` : '')
      + (showFieldNotation && fieldNotation ? ` (${fieldNotation})` : '');
  }
